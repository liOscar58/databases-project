...
public class LayoutService {
    ...
    public Pair<String, Integer> getBestDribblerDetails() throws Exception {
        ...
        String sq = "SELECT * FROM grades, students WHERE course_name = 'DRI' AND grades.student_id = students.id ORDER BY grade DESC LIMIT 1";
        String sql = "SELECT * FROM hotel, room WHERE name = 'Riverfront Retreat' AND room.hotel_id = hotel.id ORDER BY id DESC LIMIT 1";
//        String sl = "SELECT * FROM hotel WHERE category = 1 AND hotel.number_of_rooms ORDER BY grade DESC LIMIT 1";
        // connection object
        ...
        try (Connection con = db.getConnection()) {
           ...
           if (rs.next()) {
               result = new Pair<>(rs.getString("name") + " " + rs.getString("hotel_id"), rs.getInt("price"));
           }
